...
class _StudentEnrollmentTabState extends State<StudentEnrollmentTab> {
  ...
  Future<void> _resetDeviceLock(UserModel student) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppTheme.surfaceVariant,
        ...
        content: Text(
          'Are you sure you want to reset the device lock for ${student.name}? This will allow them to register a new phone on their next attendance marking.',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.textSecondary,
          ),
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: () => _resetDeviceLock(student),
                          ),
                          IconButton(
                            icon: Icon(
                              Icons.edit_outlined,
