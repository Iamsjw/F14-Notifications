...
class _ClassesTabState extends State<ClassesTab> {
  ...
  Future<void> _resetStudentDeviceLock(UserModel student, ClassModel classModel) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppTheme.surfaceVariant,
        ...
        content: Text(
          'Are you sure you want to reset the device lock for ${student.name}? This will allow them to bind a new device on their next sign-in.',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.textSecondary,
          ),
    ...
    if (confirmed == true) {
      final messenger = ScaffoldMessenger.of(context);
      final success = await SupabaseService.resetStudentDevice(student.id);
      if (success) {
        if (mounted) {
          messenger.showSnackBar(
            SnackBar(
              content: Text(
                'Device lock reset successfully for ${student.name}.',
                style: GoogleFonts.plusJakartaSans(fontSize: 13),
              ),
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _ClassCard(
                        ...
                        onResetDeviceLock: (student) => _resetStudentDeviceLock(student, classModel),
                      );
                    },
                  ),
          ),
...
class _ClassCard extends StatefulWidget {
  ...
  final Function(UserModel) onResetDeviceLock;
  ...
  const _ClassCard({
    ...
    required this.onResetDeviceLock,
  });
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(icon, size: isMobile ? 12 : 14, color: iconColor),
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      icon: Icon(
                                        ...
                                        color: student.deviceId != null && student.deviceId!.isNotEmpty
                                            ? AppTheme.warning
                                            : AppTheme.textMuted.withOpacity(0.4),
                                        size: 16,
                                      ),
                                      tooltip: student.deviceId != null && student.deviceId!.isNotEmpty
                                          ? 'Reset Device Lock (Locked)'
                                          : 'Device Unlocked',
                                      onPressed: student.deviceId != null && student.deviceId!.isNotEmpty
                                          ? () => widget.onResetDeviceLock(student)
                                          : null,
                                    ),
                                    IconButton(
                                      icon: Icon(
