...
class _AttendanceListWidgetState extends State<AttendanceListWidget> {
  ...
  Widget build(BuildContext context) {
    final attendanceMap = <String, AttendanceModel>{
      for (final a in widget.attendance) a.studentId: a
    };

    final sortedStudents = List<UserModel>.from(widget.classStudents)
      ..sort((a, b) {
    ...
    final filteredStudents = sortedStudents.where((student) {
      ...
      final rollMatch = student.rollNo?.toLowerCase().contains(q) ?? false;
      final emailMatch = student.email.toLowerCase().contains(q);
      return nameMatch || rollMatch || emailMatch;
    }).toList();

    final presentCount = widget.attendance.where((a) => a.isPresent).length;
                    ...
                    children: [
                      ...
                      AnimatedSwitcher(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            '$presentCount / ${widget.classStudents.length} present',
                            style: GoogleFonts.plusJakartaSans(
                              fontSize: 11,
                              fontWeight: FontWeight.w600,
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return RepaintBoundary(
                        ...
                        child: _AttendanceRow(
                          ...
                          index: index,
                          onToggle: (isPresent) =>
                              widget.onToggleAttendance(student.id, isPresent),
                          isSessionActive: widget.isSessionActive,
                          onRefreshStudents: widget.onRefreshStudents,
                        ),
                      );
                    },
                  ),
