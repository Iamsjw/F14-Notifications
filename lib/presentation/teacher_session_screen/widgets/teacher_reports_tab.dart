...
class _TeacherReportsTabState extends State<TeacherReportsTab> {
  ...
  Future<void> _resetStudentDeviceLock(String studentId, String studentName) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to reset the device lock for $studentName? This will allow them to register a new phone on their next attendance marking.',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
    ...
    if (confirmed == true) {
      ...
      final success = await SupabaseService.resetStudentDevice(studentId);
      if (success && mounted) {
        if (_selectedSession != null) {
          _loadSessionDetail(_selectedSession!);
        } else {
          _loadStudents();
        }
        messenger.showSnackBar(
          SnackBar(
            content: Text(
              'Device lock reset successfully for $studentName.',
        ...
        children: [
          ...
          if (_selectedSession != null) ...[
            ...
            if (_activeTab == 0) ...[
              ...
              _buildDeviceLocksFilters(),
              Expanded(child: _buildDeviceLocksView()),
            ],
          ],
        ],
      ),
    ...
    return Expanded(
      child: Column(
        children: [
          ...
          // Student Roster Search Bar
          if (_sessionAttendance.isNotEmpty)
            Padding(
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  onChanged: (val) => setState(() => _detailSearchQuery = val),
                  style: GoogleFonts.plusJakartaSans(
                    fontSize: 12,
                    color: AppTheme.textPrimary,
                  ),
                  decoration: InputDecoration(
                    hintText: 'Search student by name, roll no, email...',
          ...
          Expanded(
            child: () {
              ...
              final filteredAttendance = _sessionAttendance.where((record) {
                if (_detailSearchQuery.trim().isEmpty) return true;
                final q = _detailSearchQuery.trim().toLowerCase();
                final name = (record['users']?['name'] ?? '').toString().toLowerCase();
                final rollNo = (record['users']?['roll_no'] ?? '').toString().toLowerCase();
                final email = (record['users']?['email'] ?? '').toString().toLowerCase();
                return name.contains(q) || rollNo.contains(q) || email.contains(q);
              }).toList();
              ...
              if (filteredAttendance.isEmpty) {
                return Center(
                  child: EmptyStateWidget(
                    ...
                    description:
                        'No students found matching "$_detailSearchQuery".',
                  ),
                );
              }
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final rollNo = record['users']?['roll_no'] ?? '';
                ...
                child: Center(
                  child: Text(
                    ...
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color: _activeTab == 3 ? Colors.white : AppTheme.textMuted,
                    ),
                  ),
                ),
              ),
  ...
  Widget _buildDeviceLocksFilters() {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Row(
        children: [
  ...
  Widget _buildDeviceLocksView() {
      ...
      return Center(
        child: Padding(
          ...
          child: EmptyStateWidget(
            ...
            description: 'Choose a class to view and reset student device locks.',
          ),
        ),
      );
    }
      ...
      return Center(
        child: Padding(
          ...
          child: EmptyStateWidget(
            ...
            description: 'There are no students enrolled in this class or data is loading.',
          ),
        ),
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final isLocked = student.deviceId != null && student.deviceId!.isNotEmpty;
        return Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
              ...
              Column(
                ...
                children: [
                  Container(
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.plusJakartaSans(
                        fontSize: 9,
                        fontWeight: FontWeight.bold,
                        color: isLocked ? AppTheme.warning : AppTheme.success,
                      ),
                    ),
                  ),
                  const SizedBox(height: 8),
                  ElevatedButton.icon(
                    ...
                    onPressed: () => _resetStudentDeviceLock(student.id, student.name),
                    icon: const Icon(Icons.phonelink_erase_rounded, size: 13, color: Colors.black),
                    label: Text(
                      'Reset',
                      style: GoogleFonts.plusJakartaSans(
                        fontSize: 11,
