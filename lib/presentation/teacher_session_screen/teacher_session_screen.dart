...
class _TeacherSessionScreenState extends State<TeacherSessionScreen>
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      NotificationService.instance.startRealtimeListener(
  ...
  Future<void> _startSession() async {
    ...
    try {
      // Pre-session validation for HIGH security
      if (_securityLevel == 'HIGH') {
        final hasPermissions = await BleService.hasPermissions();
        final isBtOn = await BleService.isBluetoothOn();
        final isLocEnabled = await BleService.isLocationEnabled();
        final isPeripheral = await BleService.isPeripheralSupported();

        if (!hasPermissions || !isBtOn || !isLocEnabled || !isPeripheral) {
          setState(() {
          ...
          final dialogResult = await showDialog<dynamic>(
            ...
            builder: (ctx) => _BleSetupRequiredDialog(
              initialPermissionsGranted: hasPermissions,
              initialBluetoothOn: isBtOn,
              initialLocationServicesEnabled: isLocEnabled,
              initialPeripheralSupported: isPeripheral,
            ),
          );

          if (dialogResult == 'switch_to_low') {
            setState(() {
              _securityLevel = 'LOW';
            });
            _startSession();
          ...
          if (dialogResult != true) {
            return; // Cancelled or failed to setup
          }

          // If they successfully setup everything and pressed Start
          setState(() {
            _isStartingSession = true;
          });
        }
              ...
              delegate: SliverChildListDelegate([
                ...
                if (_activeSession != null) ...[
                  ...
                  AttendanceListWidget(
                    ...
                    onRefreshStudents: () {
                      if (_activeSession != null) {
                        _loadClassStudents(_activeSession!.classIds ?? [_activeSession!.classId]);
                      }
                    },
                  ),
                  const SizedBox(height: 16),
...
class _BleSetupRequiredDialog extends StatefulWidget {
  final bool initialPermissionsGranted;
  final bool initialBluetoothOn;
  final bool initialLocationServicesEnabled;
  final bool initialPeripheralSupported;

  const _BleSetupRequiredDialog({
    super.key,
    required this.initialPermissionsGranted,
    required this.initialBluetoothOn,
    required this.initialLocationServicesEnabled,
    required this.initialPeripheralSupported,
  });

  @override
  State<_BleSetupRequiredDialog> createState() => _BleSetupRequiredDialogState();
}

class _BleSetupRequiredDialogState extends State<_BleSetupRequiredDialog> {
  late bool _permissionsGranted;
  late bool _bluetoothOn;
  late bool _locationServicesEnabled;
  late bool _isPeripheralSupported;
  ...
  void initState() {
    ...
    _bluetoothOn = widget.initialBluetoothOn;
    _locationServicesEnabled = widget.initialLocationServicesEnabled;
    _isPeripheralSupported = widget.initialPeripheralSupported;
    _startCheckTimer();
  }
  ...
  void _startCheckTimer() {
    _statusTimer = Timer.periodic(const Duration(milliseconds: 1500), (timer) async {
      final p = await BleService.hasPermissions();
      final bt = await BleService.isBluetoothOn();
      final loc = await BleService.isLocationEnabled();
      final peri = await BleService.isPeripheralSupported();
      if (mounted) {
        setState(() {
          _permissionsGranted = p;
          _bluetoothOn = bt;
          _locationServicesEnabled = loc;
          _isPeripheralSupported = peri;
        });
      }
    });
  }
  ...
  Future<void> _fixBluetooth() async {
  ...
  @override
  Widget build(BuildContext context) {
    final allOk = _permissionsGranted && _bluetoothOn && _locationServicesEnabled && _isPeripheralSupported;
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  !_isPeripheralSupported ? Icons.warning_amber_rounded : Icons.sensors_rounded,
                  color: !_isPeripheralSupported ? AppTheme.error : AppTheme.warning,
                  size: 24,
                ),
                const SizedBox(width: 8),
                Text(
                  !_isPeripheralSupported ? 'BLE Ads Unsupported' : 'Setup Required',
                  style: GoogleFonts.plusJakartaSans(
                    fontWeight: FontWeight.w700,
                    color: AppTheme.textPrimary,
                    fontSize: 16,
            ...
            const SizedBox(height: 12),
            Text(
              !_isPeripheralSupported
                  ? 'Your device hardware does not support Bluetooth Low Energy advertising. High-security mode is unavailable.'
                  : 'To start a high-security attendance session, please resolve the following settings:',
              style: GoogleFonts.plusJakartaSans(
                fontSize: 13,
                color: AppTheme.textSecondary,
              ),
            ...
            const SizedBox(height: 16),
            if (!_isPeripheralSupported) ...[
              _buildStatusItem(
                label: 'BLE Advertising Hardware',
                isOk: false,
              ...
              _buildStatusItem(
                label: 'Permissions',
                isOk: _permissionsGranted,
                icon: Icons.shield_outlined,
                isLoading: _isFixingPermissions,
                onFix: _fixPermissions,
              ),
                ...
                if (!_isPeripheralSupported)
                  Expanded(
                    child: ElevatedButton(
                      ...
                      onPressed: () => Navigator.pop(context, 'switch_to_low'),
                      child: Text(
                        'Switch to LOW',
                        style: GoogleFonts.plusJakartaSans(
                          fontWeight: FontWeight.w600,
                  ...
                  Expanded(
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor: allOk ? AppTheme.success : AppTheme.primary,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                      ),
                      onPressed: allOk ? () => Navigator.pop(context, true) : null,
                      child: Text(
                        'Start Session',
                        style: GoogleFonts.plusJakartaSans(
                          fontWeight: FontWeight.w600,
  ...
  Widget _buildStatusItem({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isOk ? AppTheme.success.withAlpha(15) : AppTheme.warning.withAlpha(20),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isOk ? AppTheme.success.withAlpha(30) : AppTheme.warning.withAlpha(50),
        ),
      ),
      child: Row(
        children: [
          ...
          if (!isOk && showFixButton && onFix != null)
            ElevatedButton(
              style: ElevatedButton.styleFrom(
