...
import './widgets/device_reset_dialog.dart';
...
class _StudentAttendanceScreenState extends State<StudentAttendanceScreen>
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Start live Android Status Bar Push Notification listener
  ...
  Future<void> _checkDeviceBinding() async {
    if (_currentUser == null) return;
    try {
      final currentLocalId = await DeviceHelper.getUniqueDeviceId();
      final profile = await SupabaseService.getUserProfile(_currentUser!.id);

      if (profile != null && profile.deviceId != null && profile.deviceId!.isNotEmpty) {
        if (profile.deviceId != currentLocalId) {
          // Device mismatch detected pre-emptively!
          if (mounted) {
            _showDeviceResetDialog(currentLocalId);
          }
        }
      }
    } catch (e) {
      debugPrint('[StudentAttendanceScreen] Pre-emptive device check failed: $e');
    }
  }

  void _showDeviceResetDialog(String currentLocalId) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (ctx) => DeviceResetDialog(
        studentId: _currentUser!.id,
        email: _currentUser!.email,
        newDeviceId: currentLocalId,
        onResetSuccess: () {
  ...
  Future<void> _offerManualFallback(SessionModel session) async {
    if (!mounted) return;
...
class _AnimatedBellButtonState extends State<AnimatedBellButton>
  ...
  @override
  void didUpdateWidget(AnimatedBellButton oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.unreadCount > 0 && !_bellController.isAnimating) {
      _bellController.repeat(reverse: true);
    } else if (widget.unreadCount == 0 && _bellController.isAnimating) {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final angle = (widget.unreadCount > 0)
            ? (math.sin(_bellController.value * math.pi * 2) * 0.14)
            : 0.0;
        ...
        children: [
          IconButton(
            ...
            icon: Icon(
              ...
              color: widget.unreadCount > 0 ? AppTheme.primaryCyan : AppTheme.textSecondary,
              size: 22,
            ),
            tooltip: 'Notification Hub',
          ),
