...
class _SystemSetupDisclaimerState extends State<SystemSetupDisclaimer> {
  ...
  void _startAutoCheck() {
    // Periodically run check in the background every 3 seconds to avoid native bridge overhead on low-end devices
    _autoCheckTimer = Timer.periodic(const Duration(milliseconds: 3000), (timer) {
      if (mounted) {
        widget.onRefresh();
      }
    });
  ...
  Future<void> _handlePermissionsFix() async {
    ...
    try {
      ...
      debugPrint('[Setup] Failed to request permissions: $e');
    } finally {
      if (mounted) {
  ...
  Future<void> _handleBluetoothFix() async {
    ...
    try {
      ...
      // Wait a short moment for hardware radio to initialize before requesting state refresh
      await Future.delayed(const Duration(milliseconds: 600));
    } catch (e) {
      debugPrint('[Setup] Failed to enable Bluetooth: $e');
    } finally {
      if (mounted) {
  ...
  Future<void> _handleLocationFix() async {
    ...
    try {
      ...
      debugPrint('[Setup] Failed to open location settings: $e');
    } finally {
      if (mounted) {
