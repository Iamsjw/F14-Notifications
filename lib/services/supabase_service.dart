...
class SupabaseService {
  ...
  static Future<bool> markAttendance({
    ...
    try {
      // 1. Verify device binding to prevent proxy/clone usage
      ...
      if (deviceId != null && deviceId.isNotEmpty) {
        if (profile.deviceId == null || profile.deviceId!.isEmpty) {
          // Bind this device to the student account
          try {
            await client
                .from('users')
            ...
            if (kDebugMode) {
              debugPrint('[Supabase] Warning: Could not update device_id in users table: $dbErr');
            }
          }
        } else if (profile.deviceId != deviceId) {
          // Account is locked to another device - Block!
          throw Exception('This account is registered to another device. Please contact your instructor to reset it.');
        }
      }
      ...
      if (errStr.contains('registered to another device')) {
        rethrow;
      }
  ...
  /// Resets a student's bound device ID.
  /// Used by instructors/admins when a student changes their phone or reinstalls the app.
  static Future<bool> resetStudentDevice(String studentId) async {
    try {
      await client
          .from('users')
          .update({
            'device_id': null,
            'device_resets_this_month': 0,
            'last_reset_month': 0,
          })
          .eq('id', studentId);
      return true;
      ...
      if (kDebugMode) {
        debugPrint('[Supabase] resetStudentDevice failed: $e');
      }
      return false;
    }
  }

  /// Checks if a student is allowed to trigger a self-service device reset.
  /// Enforces a strict limit of 2 self-resets per calendar month.
  static Future<Map<String, dynamic>> checkSelfResetLimit(String studentId) async {
    try {
      final profile = await getUserProfile(studentId);
      if (profile == null) {
      ...
      final currentMonthInt = int.parse('${now.year}${now.month.toString().padLeft(2, '0')}');

      final dbData = await client
          .from('users')
          .select('device_resets_this_month, last_reset_month')
          .eq('id', studentId)
          .maybeSingle();

      if (dbData == null) {
        return {'allowed': true, 'remaining': 2, 'resetsCount': 0};
      }

      final lastMonth = dbData['last_reset_month'] as int? ?? 0;
      final resetsCount = dbData['device_resets_this_month'] as int? ?? 0;

      if (lastMonth == currentMonthInt) {
        final remaining = 2 - resetsCount;
        if (remaining <= 0) {
          return {
            'allowed': false,
            'remaining': 0,
            'error': 'You have exhausted your 2 self-service resets for this month. Please contact your teacher.'
          };
        }
        return {'allowed': true, 'remaining': remaining, 'resetsCount': resetsCount};
      } else {
        // New month: reset allowed
        return {'allowed': true, 'remaining': 2, 'resetsCount': 0};
      }
    } catch (e) {
      return {'allowed': false, 'error': 'Failed to verify limits: $e'};
    }
  }

  /// Triggers a password reset/recovery OTP email through Supabase Auth
  static Future<bool> sendResetDeviceOTP(String email, String studentId) async {
    try {
      final check = await checkSelfResetLimit(studentId);
      if (check['allowed'] != true) {
        throw Exception(check['error'] ?? 'Self-reset limit exceeded.');
      }
      await client.auth.resetPasswordForEmail(email.trim());
      return true;
    } catch (e) {
      if (kDebugMode) {
        debugPrint('[Supabase] sendResetDeviceOTP failed: $e');
      }
      rethrow;
    }
  }

  /// Verifies the recovery OTP and updates the student's device binding & limits
  static Future<bool> verifyDeviceResetOTP({
    required String email,
    required String studentId,
    required String token,
    ...
    try {
      // 1. Verify limit one more time before committing
      final check = await checkSelfResetLimit(studentId);
      if (check['allowed'] != true) {
        throw Exception(check['error'] ?? 'Self-reset limit exceeded.');
      }

      // 2. Verify the 6-digit OTP code using recovery channel
      ...
      // 3. Update the user record: set device_id, increment resets count
      final now = DateTime.now();
      final currentMonthInt = int.parse('${now.year}${now.month.toString().padLeft(2, '0')}');
      final currentCount = check['resetsCount'] as int? ?? 0;

      await client.from('users').update({
        'device_id': newDeviceId,
        'device_resets_this_month': currentCount + 1,
        'last_reset_month': currentMonthInt,
      }).eq('id', studentId);

      return true;
    } catch (e) {
      if (kDebugMode) {
        debugPrint('[Supabase] verifyDeviceResetOTP failed: $e');
      }
      rethrow;
    }
  }
  ...
  static Future<List<Map<String, dynamic>>> getSessionAttendanceForReport(
    ...
    try {
      ...
      final attendanceData = await client
          .from('attendance')
          .select('*, users(name, email, device_id)')
          .eq('session_id', sessionId);
